...
export class RedemptionRepositorySequelize implements RedemptionRepository {
  async save(redemption: Redemptions, transaction?: Transaction): Promise<Redemptions> {
    try {
      ...
      const completeRedemption = await RedemptionsModel.findByPk(createdRedemption.id, {
        transaction,
        include: [
      ...
      if (!completeRedemption) throw new Error("No se pudo obtener el canje completo");
